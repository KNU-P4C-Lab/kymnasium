...
class AvoidBlurpEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Dict({
            "mario": gym.spaces.Box(low=-np.inf, high=np.inf, shape=(6,)),
    ...
    def _spawn_blurp(self):
        ...
        if random.uniform(0.0, 1.0) < self.prob_spawn_on_player:
            ...
            target_x = random.randint(0, self.width - GAME_TILE_SIZE)

        if self.mode == 'vertical':
            blurp = Blurp(
                sprite=self._sprite,
                position=(target_x, 0),
                gravity=random.uniform(BLURP_GRAVITY_MIN, BLURP_GRAVITY_MAX),
    ...
    def _step_internal(self, action):
        ...
        self._character_sprites.frame_tick(dt=dt, screen_size=(self.width, self.height))
