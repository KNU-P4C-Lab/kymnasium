...
def restart():
    ...
    for pig in pigs_to_remove:
        space.remove(pig._shape, pig._shape._body)
        pigs.remove(pig)
    for bird in birds:
        birds_to_remove.append(bird)
    for bird in birds_to_remove:
        space.remove(bird._shape, bird._shape._body)
        birds.remove(bird)
    for column in columns:
        columns_to_remove.append(column)
    for column in columns_to_remove:
        space.remove(column._shape, column._shape._body)
        columns.remove(column)
    for beam in beams:
        beams_to_remove.append(beam)
    for beam in beams_to_remove:
        space.remove(beam._shape, beam._shape._body)
        beams.remove(beam)
...
while running:
    ...
    # Draw birds
    for bird in birds:
        if bird._shape._body.position.y < 0:
            birds_to_remove.append(bird)
        p = to_pygame(bird._shape._body.position)
        x, y = p
        x -= 22
        y -= 20
        screen.blit(redbird, (x, y))
        pygame.draw.circle(screen, BLUE,
                           p, int(bird._shape.radius), 2)
    ...
    for pig in pigs:
        ...
        # print (i,pig.life)
        pig = pig._shape
        if pig._body.position.y < 0:
            pigs_to_remove.append(pig)

        p = to_pygame(pig._body.position)
        x, y = p

        angle_degrees = math.degrees(pig._body.angle)
